...
def ruleParser[S](encode: ((String, String)) => State[S, Int]) =
  ...
  val oneBag = ((one <* sp) *> adjColor <* P.string("bag")).map(encode(_).map(NestedBag(_, count = 1)))

  val multiBag = ((nat <* sp) ~ adjColor <* P.string("bags")).map((n, p) => encode(p).map(NestedBag(_, count = n)))
...
def op[T](f: (Lookup, Int) => T)(adj: String, color: String)(rs: Seq[Bag], s: (Int, Encoder)): Either[String, T] =
  ...
  for id <- encoder.get((adj, color)).toRight(s"unknown bag $adj $color")
  yield f(bags, id)
...
lazy val _n =
  ...
  yield traverseState(rules)((0, Map()))
