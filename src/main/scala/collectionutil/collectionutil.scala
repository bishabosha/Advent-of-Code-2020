...
extension [S, T](st: State[S, T])

  def flatMap[U](f: T => State[S, U]): State[S, U] = s =>
    val (t, s1) = st(s)
    f(t)(s1)

  def map[U](f: T => U): State[S, U] = s =>
    val (t, s1) = st(s)
    (f(t), s1)

def occurrences[T](ts: Seq[T]) = ts.groupBy(identity).view.mapValues(_.length)

def groupLines[C[X] <: Seq[X]](lines: C[String])(using fac: Factory[Seq[String], C[C[String]]]): C[C[String]] =
  ...
  def inner(acc: mutable.Builder[Seq[String], C[C[String]]], remaining: Seq[String]): C[C[String]] =
    ...
    acc += group
    if remaining1.isEmpty then acc.result
    else inner(acc, remaining1)

  inner(fac.newBuilder, lines)

def traverse[E, T, U, C[X] <: Seq[X]](ts: C[T])(f: T => Either[E, U])(using fac: Factory[U, C[U]]): Either[E, C[U]] =
...
extension [E, T](e: Either[E, T])
  def %>[U](that: => U): Either[E, U] = e.map(_ => that)

  def *>[U](that: => Either[E, U]): Either[E, U] = e.flatMap(_ => that)

  def !>[U](f: T => U): Either[E, U] = e.map(f)

extension [T](p: T => Boolean) def &&(q: T => Boolean): T => Boolean = t => p(t) && q(t)
